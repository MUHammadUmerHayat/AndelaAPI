...
exports.returnJson = function(res, id) {
    Applicant.findById(id).where({
        _type: 'Applicant'
    }).populate('skillSet.skill').exec(function(err, user) {
        res.jsonp(user);
    });
};
...
exports.updateAssmt = function(req, res) {
    ...
    Applicant.update({
            _id: trainee._id,
            'assessments._id': assessment._id
        }, {
            $set: {
    ...
    if (fellow.role !== 'fellow') {
        ...
        Applicant.update({
            ...
            function(err) {
                if (err) {
                    res.send(500, {
                        message: 'error occurred trying to update skill rating'
                    });
                } else {
    ...
    form.parse(req, function(err, fields, files) {
        ...
        User.findById(req.user.id).exec(function(err, person) {
            if (err) {
                ...
                res.send(500, {
                    message: 'Failed to load User ' + person._id
                });
            } else {
                var experience = '';
                ...
                if (files.file) {
                    ...
                    Instructor.update({
                        ...
                        function(error) {
                            if (error) {
                                res.send(500, {
                                    message: 'Error: update operation failed'
                                });
                            } else {
                                jsonInstructor(res, person._id);
                            }
...
exports.traineeByID = function(req, res, next, id) {
    Applicant.findById(id).where({
        _type: 'Applicant'
    }).populate('campId').populate('skillSet.skill').exec(function(err, trainee) {
...
exports.isCreator = function(req, res, next) {
    if (req.assessment.instructorId.toString() !== req.user.id) {
        res.send(400, {
            message: 'You are not the creator of the assessment'
        });
    }
    next();
};
